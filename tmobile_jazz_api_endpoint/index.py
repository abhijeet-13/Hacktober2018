    ...
    try:
        ...
        if 'method' in event:
            if event['method'] == "POST":
                ...
                if 'caller' in event['query']:
                    
                    # There was some caller
                    logger.error('There was some caller: ' + event['query']['caller'])
                        
                    global state_variable
                    state_variable = 'SET'
                    data['curr_state'] = state_variable
                    
                else:
                    logger.error('Caller was someone else.')
